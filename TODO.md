...
## CR-07 Fix CI Playwright Browser Installation

- [x] **T022 · Fix · P0**: add Playwright browser installation to CI workflow

  - **Context:** CI-FAILURE-SUMMARY.md -> Root Cause Analysis
  - **Action:**
    1. Update `.github/workflows/ci.yml` to add `pnpm exec playwright install --with-deps chromium` step.
    2. Position the browser installation step after dependency installation and before Storybook a11y tests.
    3. Install only Chromium browser to optimize CI performance.
  - **Done‑when:**
    1. CI workflow includes Playwright browser installation step.
    2. Browser installation step is properly positioned in the workflow.
...
- [x] **T023 · Optimize · P1**: implement browser binary caching for CI performance
  ...
  - **Action:**
    1. Add GitHub Actions cache for Playwright browser binaries using `~/.cache/ms-playwright` path.
    2. Create cache key based on Playwright version and OS to ensure proper cache invalidation.
    3. Monitor cache hit rates and CI performance impact.
  - **Done‑when:**
    1. Browser binary caching is implemented in CI workflow.
    2. Cache hit rate exceeds 80% for repeat builds.
    3. CI duration increase is minimized to <2 minutes.
  - **Verification:**
    1. Test cache behavior with multiple CI runs.
...
- [ ] **T025 · Docs · P2**: update documentation for CI browser requirements
  ...
  - **Action:**
    ...
    2. Add troubleshooting section for browser installation failures.
    3. Document CI performance impact and optimization strategies.
    4. Update `CONTRIBUTING.md` to mention browser dependency for a11y tests.
  - **Done‑when:**
    1. Documentation clearly explains browser installation requirement.
    2. Troubleshooting guidance is available for common issues.
    3. Performance impact is documented with mitigation strategies.
    4. Contributors understand the CI browser dependency.
  - **Verification:**
    1. Review documentation for clarity and completeness.
    2. Ensure troubleshooting steps are actionable.
  - **Depends‑on:** [T024]
```
